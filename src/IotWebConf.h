...
#define IOTWEBCONF_CONFIG_START 285
#endif
...
const char IOTWEBCONF_HTML_FORM_START[] PROGMEM =
    "<form action='' method='post'><fieldset><input type='hidden' "
    "name='iotSave' value='true'>";
const char IOTWEBCONF_HTML_FORM_PARAM[] PROGMEM =
    "<div class='{s}'><label for='{i}'>{b}</label><input type='{t}' id='{i}' "
    "name='{i}' maxlength={l} placeholder='{p}' value='{v}' {c}/><div "
    "class='em'>{e}</div></div>";
const char IOTWEBCONF_HTML_FORM_END[] PROGMEM =
    "</fieldset><button type='submit'>Apply</button></form>";
const char IOTWEBCONF_HTML_SAVED[] PROGMEM =
    "<div>Configuration saved<br />Return to <a href='/'>home page</a>.</div>";
...
class IotWebConfParameter {
 public:
   ...
   */
  IotWebConfParameter(const char *label, const char *id, char *valueBuffer,
                      int length, const char *type = "text",
                      const char *placeholder = NULL,
                      const char *defaultValue = NULL,
                      const char *customHtml = NULL, boolean visible = true);
   ...
   */
  IotWebConfParameter(const char *id, char *valueBuffer, int length,
                      const char *customHtml, const char *type = "text");
...
class IotWebConf {
 public:
   ...
   */
  IotWebConf(const char *thingName, DNSServer *dnsServer, WebServer *server,
   ...
   */
  void setupUpdateServer(HTTPUpdateServer *updateServer,
   ...
   * changed. Should be called before init()!
   */
   ...
   * IotWebConf::connectAp() as reference when implementing your custom
   * solution.
   */
   ...
   * IotWebConf::connectWifi() as reference when implementing your custom
   * solution.
   */
   ...
   * reference implementation IotWebConf::handleConnectWifiFailure(). This means
   * we need to fall back to AP mode. If it method returns with a (new) WiFi
   * settings, it is used as a next try. Note, that this feature is provided
   * because of a possible future option of providing multiply WiFi settings.
   */
  void setWifiConnectionFailedHandler(
      std::function<IotWebConfWifiAuthInfo *()> func) {
  ...
  /**
   * By default IotWebConf starts up in AP mode. Calling this method before the
   * init will force IotWebConf to connect immediately to the configured WiFi
  ...
  /**
   * By default IotWebConf will continue startup in WiFi mode, when no
   ...
   * this method. Note, that init() must pretend configSave()! Also note, that
   * configSave writes to EEPROM, and EEPROM can be written only some thousand
   ...
   */
  void setHtmlFormatProvider(
      IotWebConfHtmlFormatProvider *customHtmlFormatProvider) {
 ...
 private:
  ...
  IotWebConfHtmlFormatProvider *htmlFormatProvider =
      &htmlFormatProviderInstance;

...
IotWebConfParameter::IotWebConfParameter(const char* id, char* valueBuffer,
                                         int length, const char* customHtml,
                                         const char* type) {
  this->label = NULL;
  this->_id = id;
  this->valueBuffer = valueBuffer;
...
IotWebConf::IotWebConf(const char* defaultThingName, DNSServer* dnsServer,
                       WebServer* server, const char* initialApPassword,
                       const char* configVersion) {
  strncpy(this->_thingName, defaultThingName, IOTWEBCONF_WORD_LEN);
  strcpy(_apPassword, "plantOSdevice");
  ...
  this->_thingNameParameter = IotWebConfParameter(
      "Thing name", "iwcThingName", this->_thingName, IOTWEBCONF_WORD_LEN);
  this->_apPasswordParameter =
      IotWebConfParameter("AP password", "iwcApPassword", this->_apPassword,
                          IOTWEBCONF_PASSWORD_LEN, "password");
  this->_wifiSsidParameter = IotWebConfParameter(
      "WiFi SSID", "iwcWifiSsid", this->_wifiSsid, IOTWEBCONF_WORD_LEN);
  this->_wifiPasswordParameter = IotWebConfParameter(
      "WiFi password", "iwcWifiPassword", this->_wifiPassword,
      IOTWEBCONF_PASSWORD_LEN, "password");
  this->_apTimeoutParameter = IotWebConfParameter(
      "Startup delay (seconds)", "iwcApTimeout", this->_apTimeoutStr,
      IOTWEBCONF_WORD_LEN, "number", NULL, NULL, "min='1' max='600'", false);
...
char* IotWebConf::getThingName() { return this->_thingName; }

void IotWebConf::setConfigPin(int configPin) { this->_configPin = configPin; }

void IotWebConf::setStatusPin(int statusPin) { this->_statusPin = statusPin; }

void IotWebConf::setupUpdateServer(HTTPUpdateServer* updateServer,
...
boolean IotWebConf::configLoad() {
  if (this->configTestVersion()) {
    ...
    Serial.println((String) "iotwebconf: reading at offset " + start);
    while (current != NULL) {
      if (current->getId() != NULL) {
        this->readEepromValue(start, current->valueBuffer,
                              current->getLength());
#ifdef IOTWEBCONF_DEBUG_TO_SERIAL
        const char* defaultMarker = "";
#endif
        if ((strlen(current->valueBuffer) == 0) &&
            (current->defaultValue != NULL)) {
          strncpy(current->valueBuffer, current->defaultValue,
                  current->getLength());
#ifdef IOTWEBCONF_DEBUG_TO_SERIAL
...
void IotWebConf::handleConfig() {
  if (this->_state == IOTWEBCONF_STATE_ONLINE) {
    // -- Authenticate
    if (!this->_server->authenticate(IOTWEBCONF_ADMIN_USER_NAME,
                                     this->_apPassword)) {
  ...
  if (!this->_server->hasArg("iotSave") || !this->validateForm()) {
    ...
    while (current != NULL) {
      if (current->getId() == NULL) {
        ...
        if (current->label != NULL) {
          ...
          pitem.replace(
              "{p}", current->placeholder == NULL ? "" : current->placeholder);
          ...
          pitem.replace("{c}",
                        current->customHtml == NULL ? "" : current->customHtml);
          pitem.replace("{e}", current->errorMessage == NULL
                                   ? ""
    ...
    while (current != NULL) {
      if ((current->getId() != NULL) && (current->visible)) {
        if ((strcmp("password", current->type) == 0) &&
          ...
          this->readParamValue(current->getId(), current->valueBuffer,
                               current->getLength());
#ifdef IOTWEBCONF_DEBUG_TO_SERIAL
          Serial.print(current->getId());
          Serial.print("='");
    ...
    if (this->_apPassword[0] == '\0') {
      page +=
          F("You must change the default AP password to continue. Return "
            "to <a href=''>configuration page</a>.");
    } else if (this->_wifiSsid[0] == '\0') {
      page +=
          F("You must provide the local wifi settings to continue. Return "
            "to <a href=''>configuration page</a>.");
...
void IotWebConf::readParamValue(const char* paramName, char* target,
                                unsigned int len) {
...
boolean IotWebConf::validateForm() {
  ...
  // -- Internal validation.
  int l = this->_server->arg(this->_wifiPasswordParameter.getId()).length();
  if ((0 < l) && (l < 8)) {
...
void IotWebConf::handleNotFound() {
  ...
  this->_server->sendHeader("Cache-Control",
...
boolean IotWebConf::handleCaptivePortal() {
  ...
  if (!isIp(host) && !host.startsWith(thingName)) {
    ...
    this->_server->sendHeader(
        "Location",
        String("http://") + toStringIp(this->_server->client().localIP()),
        true);
    this->_server->send(302, "text/plain",
                        "");  // Empty content inhibits Content-length header so
                              // we have to close the socket ourselves.
    this->_server->client()
...
void IotWebConf::changeState(byte newState) {
  switch (newState) {
    case IOTWEBCONF_STATE_AP_MODE: {
      // -- In AP mode we must override the default AP password. Otherwise we
      // stay in STATE_NOT_CONFIGURED.
...
void IotWebConf::stateChanged(byte oldState, byte newState) {
  ...
  switch (newState) {
    ...
    case IOTWEBCONF_STATE_CONNECTING:
      ...
      this->_wifiConnectionHandler(this->_wifiAuthInfo.ssid,
                                   this->_wifiAuthInfo.password);
      break;
    case IOTWEBCONF_STATE_ONLINE:
      this->blinkInternal(8000, 2);
      if (this->_updateServer != NULL) {
        this->_updateServer->updateCredentials(IOTWEBCONF_ADMIN_USER_NAME,
                                               this->_apPassword);
      }
      this->_server->begin();
...
boolean IotWebConf::checkWifiConnection() {
  if (WiFi.status() != WL_CONNECTED) {
    if (this->_wifiConnectionTimeoutMs <
        millis() - this->_wifiConnectionStart) {
  ...
  if (doForce) {
    ...
    if (this->_state == IOTWEBCONF_STATE_AP_MODE) {
      if (this->mustStayInApMode()) {
        IOTWEBCONF_DEBUG_LINE(
            F("Requested stopping to force AP mode, but we cannot leave the AP "
              "mode now."));
      } else {
...
IotWebConfWifiAuthInfo* IotWebConf::handleConnectWifiFailure() { return NULL; }
